  ...
  const components = {
    pre: ({ children, ...props }: React.ComponentProps<"pre">) => {
      ...
      if (React.isValidElement(children)) {
        ...
        codeContent = (children.props as { children?: React.ReactNode })
          ?.children;
      }
    ...
    code: ({ children, className, ...props }: React.ComponentProps<"code">) => {
      ...
      return (
        <code
          className="bg-[#1e1e1e] px-2 py-1 rounded-md text-sm font-mono text-white border border-slate-700"
          {...props}
        >
          {children}
        </code>
          ...
          <nav className="p-4">
            ...
            <ul className="space-y-2">
              {headings.map((h) => (
                <li
                  key={h.id}
                  style={{ marginLeft: `${(h.depth - 1) * 16}px` }}
                >
                  <a
                    href={`#${h.id}`}
